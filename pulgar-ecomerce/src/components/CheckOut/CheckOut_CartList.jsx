...
const CheckOut_CartList = ({ cartList, totalPrice }) => {
  const totalCant = cartList.reduce(
  ...
  return (
    <div className="col-md-5">
      <h4 className="d-flex justify-content-between align-items-center mb-3">
        ...
        <span className="badge bg-primary rounded-pill">{totalCant}</span>
      </h4>
      <ul className="list-group mb-3">
        ...
        {cartList.map((product) => (
          <li
            key={product.id}
            className="list-group-item d-block justify-content-between lh-sm"
          >
            {/* Detalles del producto en el carrito */}
            <div>
              <h6 className="my-0">
                {product.name} | {product.cant} unid.
              </h6>
